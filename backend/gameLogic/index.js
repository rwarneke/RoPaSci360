...
class Game {
	...
	constructor() {
		...
		/*
		RaPoSci360 is a two player *simultaneous turn* game.
	...
	static emptyBoard() {
		...
		Note that in RaPoSci360, hexes can be occupied by multiple pieces.
	...
	executeMoves() {
		...
		// move the tokens
		var toHexes = [];
		[this.nextMoves.Lower, this.nextMoves.Upper].forEach((move) => {
			console.log(move);
			// the move is assumed to be legal
			const { player, actionType, fromHex, tokenType, toHex } = move;
			this.board[toHex][tokenType] += 1;
			...
			if (actionType == ACTION_THROW) {
				...
				this.board[fromHex][tokenType] -= 1;
			}
		});

		// capture tokens
		// we only need to check the toHexes
		toHexes.forEach((toHex) => {
